...
export const useReviewStore = defineStore('review', () => {
  ...
  /** 复习词库总览：全部已学词（按到期时间升序，含词条映射），供复习页列表使用 */
  async function loadAll() {
    const cards = await cardRepo.getAll()
    const all = await wordRepo.getAll()
    const map = new Map(all.map((w) => [w.id, w]))
    // 与 load() 一致：清理词条已不存在的孤儿卡
    const orphaned = cards.filter((card) => !map.has(card.wordId))
    if (orphaned.length > 0) await cardRepo.removeByWordIds(orphaned.map((card) => card.wordId))

    const items: ReviewItem[] = []
    for (const card of cards) {
      const word = map.get(card.wordId)
  ...
  return { queue, total, index, finished, current, currentWord, inSession, reviewList, load, loadAll, rate, skip }
})
